...
def signif(samples,vararray, signal,  dirname, selprearray=None, hist_config=None, legend_config=None, extratag = ""):
    ...
    for sel in selarray:
        ...
        for var in vararray:
            ...
            if (os.path.isfile(dirname + '/' + savename + '.log') and (x)):
                ...
                sigrep = "Make "+signal+" hist"
                lookfor = ["TOTAL", sigrep, signal]
                #print lookfor
                with open(f_table, 'w+') as f:# W+ creates the new file for writing
                    nline = 0
                    for i in range(len(lookfor)):
                        #print lookfor[i]
                        command = "grep \""+ str(lookfor[i]) +"\" " + str(dirname + '/' + savename + '.log |  grep +/- >> ') + str(f_table)
                       
                        os.system(command)
                        if (lookfor[i] == sigrep):
                            command = "grep \"" + sigrep +"\" " + str(dirname + '/' + savename + '.log  >> ') + str(f_table)
                            #print command
                            os.system(command)
    ...
    with open(ftable, 'w+') as f:
        ...
        for j in range(len(xname)):
            ...
            f.write('%15s   %5f    %5f  %5f  %5f  %5s' %(xname[j],  total_b[j], total_b_err[j] , total_s[j] , total_s_err[j] , sig_cut_for_plots[j]))
...
def makesob_plots(calcsig, sigstr_BCuts, samples,  dirname,  vararray, signal,selarray=None, hist_config=None, legend_config=None, extratag = "" ):
    ...
    xname_all = [] # cut names for x lables
    #signame = []
    sig_all = [] 
    sig_err_all = []
    bkg_all = []
    ...
    xname = []
    #signame = []
    sig = [] # it is alist of lists for significanse for all the signal samples
    sig_err = [] # it is alist of lists for significanse for all the signal samples
    ...
    #if (debug): 
    print "sgnig_max  " + str(signif_max)
    ...
    if (debug): print sig_all[1]    
    if (debug): print sig[1]
    

    #n= int(len(xname))
    #nsig= int(len(signame))
    nhist = ["_sig", "_bkg", "_bkgsqrt", "_sob", "_sosqrtb"]
    if (debug): print "How many cuts ?   " + str(len(xname[0]))
    '''
...
#for i in range(len(signame)):
    if (debug): print "Histogram " + signal
    n = int(len(xname))
    ...
    for k in range(len(nhist)):
        for i in range(len(xname)):# loop on cuts sets
            if (debug): print "signal  and cut set  " + str(j) +"  " + str(i)
            if (debug): print xname[i] + "has significance" +  str(ysig[i])#first index is signal sample, second index is cut
                #sig = round(float(ysig[i]),2)
            if (debug): print str(i) + "   sig[i]   " + str(sig[i])
            #if (debug): 
            #print histall[k].GetName()
            if (debug): print nhist[k] 
            ...
            histall[k].GetXaxis().SetBinLabel(n+1, "X" + xname_all[bscut]) # notice it is from the initial, bigger cutlist
            ...
            if (nhist[k]=='_bkg'): 
                ...
                if (debug): print histall[k].GetBinContent(i+1)
            if (nhist[k]=='_bkgsqrt'): 
                ...
                if (debug): print histall[k].GetBinContent(i+1)
                if (debug): print Sigh.GetXaxis().GetBinLabel(i+1)
    gStyle.SetOptStat(0);  
    ...
    for k in range(len(nhist)):
        if (nhist[k]=="_sob"):
            ...
            if (debug): print "sob" + str(histall[k].GetBinContent(1))
        if (nhist[k]=="_sosqrtb"):
        ...
        if (debug): print histall[k].GetXaxis().GetBinLabel(i+1)
    ...
    for k in range(len(nhist)):
        ...
        with open(ftable, 'w+') as f:
            ...
            for j in range(len(xname)):
                #print signal
                print ('%15s   %5s    %5s  %5s  %5s  %5s' %(str(xname[j]),  str(bkg[j]), str(bkg_err[j]) , str(sig[j]) , str(sig_err[j]) , sig_cut_for_plots[j]) )   
